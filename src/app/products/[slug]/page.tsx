...
export default function CategoryPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const categoryDisplayName = matchedCategory?.name || slugToCategoryName(slug)
    .split(' ')
  ...
  return (
    <div className="min-h-screen bg-gray-50/50 dark:bg-slate-950 text-gray-900 dark:text-gray-100 flex flex-col font-sans">
      ...
      <main className="flex-1 max-w-7xl mx-auto w-full px-4 py-4 pb-12">
        ...
        {/* Category Pills Bar — for navigating to other categories */}
        <div className="flex gap-2 overflow-x-auto no-scrollbar pb-3 mb-3">
          ...
          {categories.map((cat) => (
            <Link
              key={cat.id}
              href={`/products/${cat.slug || cat.name.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '')}`}
        ...
        {isLoading ? (
          ...
          <div className="py-16 text-center space-y-4">
            ...
            <p className="text-gray-500 text-sm">
              আমাদের সকল প্রোডাক্ট দেখতে All Products-এ যান।
            </p>
            <Link
              href="/products"
              className="inline-block px-6 py-2 bg-black dark:bg-white text-white dark:text-black font-bold text-xs rounded-xl cursor-pointer"
            >
              সব প্রোডাক্ট দেখুন
            </Link>
          </div>
                ...
                <div className="space-y-2 pt-3 border-t border-gray-100 dark:border-slate-800">
                  <div className="flex justify-between text-xs font-bold">
                    ...
                    <span className="text-gray-900 dark:text-white">
                      {storeConfig.currency}{maxPrice.toLocaleString()}
                    </span>
                  </div>
                  <input
                    type="range"
